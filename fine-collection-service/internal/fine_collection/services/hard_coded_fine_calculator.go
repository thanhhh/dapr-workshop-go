...
type hardCodedFineCalculator struct {
}

func NewFineCalculator() fc.FineCalculator {
	return &hardCodedFineCalculator{}
}

func (c hardCodedFineCalculator) CalculateFine(licenseKey string, violationInKmh int) (int, error) {
	if licenseKey == "" ||
		licenseKey != "HX783-K2L7V-CRJ4A-5PN1G" {
		return 0, errors.ErrLicenseKeyInvalidOrNotFound
	}

	fine := 9 // default administration fee
	if violationInKmh < 5 {
		fine += 18
