...
1. Create the file `dapr/components/statestore.yaml` in VS Code.
...
2. Inspect the code in the `VehicleEntry` and `VehicleExit` methods of this handler. The methods refer to a `repository` which is defined earlier in the file:
